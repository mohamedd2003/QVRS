...
const navigate=useNavigate()
  const validationSchema = yup.object({
    ...
    email: yup
      .string()
      .matches(emailRegex, "يرجى إدخال بريد إلكتروني صحيح")
      .required("حقل مطلوب: يرجى إدخال البريد الإلكتروني"),
...
const handleSubmit=(values)=>{
  setLoading(true)
axios.post('https://vercel-api-deployment-iota.vercel.app/auth/register',values)
.then((res)=>
  {
    ...
    .catch(
      (err)=>{
        setLoading(false)
        if(err.response.data.detail==="Username or email already registered"){
          toast.error("اسم المستخدم او البريد الالكتروني مسجل سابقا")
        }
        else{
...
<section id='Register' className='p-10 bg-cover  bg-[url("https://res.cloudinary.com/dnmwmrxmr/image/upload/f_auto,q_auto/cayzt5zecsl3r1ftojuq")]'>
...
<form className="md:px-10 mx-auto" onSubmit={formik.handleSubmit}>
  ...
  <label htmlFor="email" className="block mb-2 text-lg font-medium text-white dark:text-white mt-3">البريد الالكتروني</label>
  <div className="flex">
    <span className="inline-flex items-center px-3 text-sm text-green-900 bg-green-200 border border-e-0 border-green-300 rounded-s-md dark:bg-green-600 dark:text-green-400 dark:border-green-600">
    <svg className="w-4 h-4  text-green-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 16">
    ...
    id="email" className="rounded-none rounded-e-lg bg-green-50 border border-green-300 text-green-900 focus:ring-green-500 focus:border-green-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-green-700 dark:border-green-600 dark:placeholder-green-400 dark:text-white dark:focus:ring-green-500 dark:focus:border-green-500" placeholder="ادخل البريد الالكتروني"/>
  </div>
  {formik.touched.email && formik.errors.email ? (
         <div className='block mt-3 bg-red-600 text-white font-normal text-lg p-2 rounded-lg'>{formik.errors.email}</div>
       ) : null}
...
<button type="submit"  disabled={!formik.isValid || !formik.dirty} className=' flex float-end mt-5 border-white
   hover:border-green-600 hover:bg-gradient-to-r focus:from-green-500
    focus:bg-slate-500 focus:transition focus:duration-200 hover:from-green-500
     ...
     <svg aria-hidden="true" class="inline w-6 mx-2 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-green-900" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<button type="submit"  disabled={!formik.isValid || !formik.dirty} className=' float-end mt-5 border-white
   hover:border-green-600 hover:bg-gradient-to-r focus:from-green-500
    focus:bg-slate-500 focus:transition focus:duration-200 hover:from-green-500
     hover:bg-slate-500 hover:transition hover:duration-200  font-bold border-2 border-solid
      text-white py-3 px-4 rounded-full'>انشاء حساب جديد</button>
    } 
